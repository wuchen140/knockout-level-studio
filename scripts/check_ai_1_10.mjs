...
const output = path.join(root, 'output/ai-1-10-20260909');
...
function structuralChecks(level, original) {
  ...
  assert.equal(level.design.revision, '2026-09-09-platform-locked', label);
  assert.equal(level.settings.difficulty, 0, label);
  ...
  assert.deepEqual(level.platforms, original.platforms, `${label}: platforms differ from mainline`);
  assert.equal(level.settings.moveCount, original.settings.moveCount, `${label}: onboarding move budget differs from mainline`);
...
for (let id = 1; id <= 10; id++) {
  ...
  for (const group of platformGroups) {
    const intended = shots.find(shot => shot.platform === group.sequence && shot.kind === 'intended' && shot.strength === 20);
    if (!intended || intended.moved < 1) failures.push(`${id}: intended hit has no observable response on platform ${group.sequence}`);
  }
  report.push(row);
...
fs.writeFileSync(path.join(output, 'physics-report.json'), JSON.stringify({
  revision: '2026-09-09-platform-locked',
...
console.log('PASS: AI1-10 materials, catalog, index, rotated platform bounds, oriented overlap, group supports and five-second idle. Both impact targets tested on every platform.');
